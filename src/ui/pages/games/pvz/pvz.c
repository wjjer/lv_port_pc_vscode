...
#define  zidan_speed   50    //子弹每50ms飞一个像素 (优化性能，平衡流畅度)
...
void pvz_show(void) {
    ...
    lv_obj_set_size(game_page, LV_HOR_RES, LV_VER_RES);
    lv_obj_set_style_bg_color(game_page, lv_color_hex(0x000000), 0);
    lv_obj_remove_flag(game_page, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_scroll_dir(game_page, LV_DIR_NONE);

    map1=lv_img_create(game_page);
    lv_img_set_src(map1, &pvz_map_1);
    lv_obj_clear_flag(map1, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_scroll_dir(map1, LV_DIR_NONE);
    ...
    // 退出按钮（在game_page上，避免被map1覆盖）
    botton_exit=lv_btn_create(game_page);
    lv_obj_set_style_bg_color(botton_exit,lv_color_hex(0x6f3011), LV_PART_MAIN);
    lv_obj_set_style_radius(botton_exit, 8, 0);
    lv_obj_align(botton_exit, LV_ALIGN_TOP_RIGHT, -10, 10);
    lv_obj_set_size(botton_exit, 70, 35);
    lv_obj_add_event_cb(botton_exit,exit_game_cb,LV_EVENT_CLICKED,0);
    lv_obj_t * exit_lable=lv_label_create(botton_exit);
    ...
    timer_newzb=lv_timer_create(timer_cb1, zb_period, NULL);
    timer_movezb=lv_timer_create(timer_cb2, zb_move_speed, NULL);
    timer_newshine=lv_timer_create(timer_cb3, shine_period, NULL);
    timer_zidan_fly=lv_timer_create(zidan_move, zidan_speed, NULL);
}
...
void pvz_hide(void) {
    ...
    // 删除定时器
    if(timer_newzb) { lv_timer_del(timer_newzb); timer_newzb = NULL; }
    if(timer_movezb) { lv_timer_del(timer_movezb); timer_movezb = NULL; }
    if(timer_newshine) { lv_timer_del(timer_newshine); timer_newshine = NULL; }
    if(timer_zidan_fly) { lv_timer_del(timer_zidan_fly); timer_zidan_fly = NULL; }
    if(timer_led) { lv_timer_del(timer_led); timer_led = NULL; }

    // 删除游戏页面（会连带删除所有子对象和动画）
    if(game_page) {
...
// 倭瓜死亡简化动画回调（合并位置移动和淡出）
static void wogua_dead_simple_cb(void * var, int32_t v)
{
    wogua_type * user = (wogua_type *)var;
    if(user == NULL || user->wogua == NULL) return;

    // 0-50: 位置移动 (x+40, y+20)
    if(v < 50) {
        lv_obj_set_pos(user->wogua, user->x * 49 + 18 + v * 4 / 5, user->y * 54 + 44 + v * 2 / 5);
    }
    // 50-100: 淡出
    else {
        lv_obj_set_style_img_opa(user->wogua, 255 - (v - 50) * 5, 0);
    }
}
...
{
	int x,y,i,j;
	wandou_type * user=(wandou_type *)lv_timer_get_user_data(t);
	x=user->x;
	y=user->y;
						...
						if((wogua[j].live>0&&zb_matrix[i].y==wogua[j].y)&&(zb_matrix[i].x-wogua[j].x*49<80))
							 {
								...
								// 简化倭瓜死亡动画：1个动画替代原来的3个
								lv_anim_t a4;
								lv_anim_init(&a4);
								lv_anim_set_var(&a4, &wogua[j]);
								lv_anim_set_exec_cb(&a4, wogua_dead_simple_cb);
								lv_anim_set_path_cb(&a4, lv_anim_path_ease_in);
								lv_anim_set_time(&a4, 500);
								lv_anim_set_values(&a4, 0, 100);
...
{
	int x,y,i,j;
	sunflower_type * user=(sunflower_type *)lv_timer_get_user_data(t);
	x=user->x;
	y=user->y;
